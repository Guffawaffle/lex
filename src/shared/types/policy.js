...
export function validatePolicyModule(module) {
    ...
    if (m.owns_paths !== undefined) {
        if (!Array.isArray(m.owns_paths))
            return false;
        if (!m.owns_paths.every((p) => typeof p === 'string'))
            return false;
    }
    if (m.owns_namespaces !== undefined) {
        if (!Array.isArray(m.owns_namespaces))
            return false;
        if (!m.owns_namespaces.every((n) => typeof n === 'string'))
            return false;
    }
    if (m.exposes !== undefined) {
        if (!Array.isArray(m.exposes))
            return false;
        if (!m.exposes.every((e) => typeof e === 'string'))
            return false;
    }
    if (m.coords !== undefined) {
        ...
        if (m.coords.length !== 2)
            return false;
        if (typeof m.coords[0] !== 'number' || typeof m.coords[1] !== 'number')
            return false;
    }
    if (m.allowed_callers !== undefined) {
        if (!Array.isArray(m.allowed_callers))
            return false;
        if (!m.allowed_callers.every((c) => typeof c === 'string'))
            return false;
    }
    if (m.forbidden_callers !== undefined) {
        if (!Array.isArray(m.forbidden_callers))
            return false;
        if (!m.forbidden_callers.every((c) => typeof c === 'string'))
            return false;
    }
    if (m.feature_flags !== undefined) {
        if (!Array.isArray(m.feature_flags))
            return false;
        if (!m.feature_flags.every((f) => typeof f === 'string'))
            return false;
    }
    if (m.requires_permissions !== undefined) {
        if (!Array.isArray(m.requires_permissions))
            return false;
        if (!m.requires_permissions.every((p) => typeof p === 'string'))
            return false;
    }
    if (m.kill_patterns !== undefined) {
        if (!Array.isArray(m.kill_patterns))
            return false;
        if (!m.kill_patterns.every((k) => typeof k === 'string'))
            return false;
    }

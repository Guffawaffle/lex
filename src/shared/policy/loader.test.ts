...
describe('loadPolicy', () => {
  ...
  test('custom path overrides default', () => {
    ...
    const customPath = resolve(__dirname, '../../../src/policy/policy_spec/lexmap.policy.json');
  ...
  test('environment variable works for custom path', () => {
    ...
    try {
      process.env.LEX_POLICY_PATH = resolve(__dirname, '../../../src/policy/policy_spec/lexmap.policy.json');
      const policy = loadPolicy();

...
function ingestAtlasData(
  ...
  const ingestTransaction = db.transaction(
    (runData: CodeAtlasRun, unitData: CodeUnit[]) => {
      // Save the run first
      ...
      const ingestedCount = validUnits.length > 0 ? saveCodeUnitsBatch(db, validUnits) : 0;

      return { unitsIngested: ingestedCount, unitsSkipped: skippedCount };
    }
  );

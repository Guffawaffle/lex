...
> **Status:** Historical — describes the original 1.0 planning contract, not current behavior
> **Created:** 2025-11-27
> **Chief Architect Review:** Completed

Current Frame and store semantics are defined by
[`docs/CONTRACT_SURFACE.md`](./CONTRACT_SURFACE.md) and
[`src/memory/store/CONTRACT.md`](../src/memory/store/CONTRACT.md). The ULID and lifecycle material
below is retained only as historical decision context.
...
1. **User runs CLI command** (in LexRunner)
   - Command: `lexrunner run --plan plan.json`

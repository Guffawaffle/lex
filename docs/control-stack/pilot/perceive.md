...
### 4.4 The SAFE Task Invariant

> **Invariant:** Perceive must always produce at least one SAFE task class that can run.

Even when $a > \theta$ or $\delta < \delta_{\min}$:
- SAFE tasks (read-only, summarization, dry-run) are still allowed
- Only MUTATING tasks are gated

If truly nothing safe can be identified, Perceive must emit an explicit error:

```typescript
interface PerceiveError {
  type: "no_safe_tasks";
  reason: string;
  raw_request: string;
}
...
This ensures the loop always has forward progress, even under uncertainty.

---

## 5. Allowed Task Classes

Perceive outputs not just intent, but which task classes are permitted:

$$
\text{allowed\_classes} = \begin{cases}
[\texttt{safe}, \texttt{mutating}] & \text{if } a \leq \theta \text{ and } \delta \geq \delta_{\min} \\
[\texttt{safe}] & \text{otherwise}
\end{cases}
$$

Layout will only schedule tasks matching these classes.

---

## 6. Risk Mode as Pure Function
...
export const PerceivedIntent = z.object({
  ...
  // Task class gating (new in v0)
  allowed_task_classes: z.array(z.enum(["safe", "mutating"])).min(1),

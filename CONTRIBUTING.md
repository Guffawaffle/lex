...
```
lex/
├── src/                      # Runtime and library source
│   ├── cli/                  # Compatibility barrel
...
├── canon/                    # Canonical tracked inputs
│   ├── prompts/              # Prompt templates
│   ├── schemas/              # JSON schemas
│   ├── instructions/         # Canonical assistant instructions
│   ├── policy/               # Canonical repository policy
...
└── test/                     # Tests and shared fixtures
```

**Key principles:**
- Generated JavaScript belongs in `dist/`; the source guard rejects generated `.js`/`.jsx` files
  while retaining explicitly reviewed source exceptions such as the deprecated MCP `.mjs` shim.
- `canon/` is the **source of truth** for packaged schemas, prompts, and other canonical inputs.
...
The checked-in `lint-baseline.json` is the source of truth. Do not copy a static warning count into
documentation; the budget is expected to shrink as warnings are removed.
...
# Add a repository-wide, check-only Prettier audit
npm run local-ci -- --pretty

# Exact alias
npm run local-ci -- --prettier
```

The default local CI gate does not audit every repository file with Prettier. `--pretty` and
`--prettier` add that check without rewriting files. Use `npm run format` only when you intend to
...
For every new or changed public entry point, also update `docs/PUBLIC_API.md` and
`scripts/public-api-contract.mjs`, then run:
...
- Place tests under `test/` in the corresponding source area.
...
Follow the prompts:
1. Select packages affected (usually just `@smartergpt/lex`)
...
# Verify the candidate; npm publication remains a human-only checkpoint
npm run ci:full
npm run test:smoke
```
...
3. **Run final release and packed-consumer gates**
4. **Human publishes the reviewed npm artifact**
5. **Create and push the signed Git tag** (format: `vX.Y.Z`)
6. **Verify the GitHub Release and protected Registry workflow**

**Tag format:** `vX.Y.Z` (for example, `v4.0.0`)
...
8. **Generated-source guard** - `npm run guard:no-js-src`

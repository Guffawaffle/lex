...
function extractVariables(template) {
  ...
  // Remove {{#each}} blocks
  const cleanedTemplate = template.replace(/\{\{#each\s+[a-zA-Z_][a-zA-Z0-9_\.]*\s*\}\}[\s\S]*?\{\{\/each\}\}/g, '');
...
function main() {
  ...
  console.log(`${GREEN}✓ ${files.length - results.filter(r => r.errors.length > 0).length} passed${RESET}`);

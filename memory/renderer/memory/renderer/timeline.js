...
export function buildTimeline(frames) {
    ...
    const sortedFrames = [...frames].sort((a, b) => new Date(a.timestamp).getTime() - new Date(b.timestamp).getTime());
    const timeline = [];
    let previousModules = new Set();
    let previousBlockers = new Set();
 ...
 */
export function filterTimeline(timeline, options) {
    let filtered = timeline;
    if (options.since) {
        filtered = filtered.filter(entry => new Date(entry.frame.timestamp) >= options.since);
    }
    if (options.until) {
        filtered = filtered.filter(entry => new Date(entry.frame.timestamp) <= options.until);
    }
    return filtered;
}
/**
 ...
 */
export function renderTimelineText(timeline, title) {
 ...
 */
function getStatusIndicator(frame) {
 ...
 */
export function renderModuleScopeEvolution(timeline) {
    if (timeline.length === 0) {
        return '';
    }
    const lines = [];
    ...
    const moduleAppearances = new Map();
    ...
    for (const [module, appearances] of Array.from(moduleAppearances.entries()).sort()) {
        ...
        const graph = Array.from({ length: timeline.length }, (_, i) => appearances.has(i) ? '█' : ' ').join('');
 ...
 */
export function renderBlockerTracking(timeline) {
    if (timeline.length === 0) {
        return '';
    }
    const lines = [];
 ...
 */
export function renderTimelineJSON(timeline) {
    return JSON.stringify(timeline, null, 2);
}
/**
 * Render timeline as HTML (basic implementation)
 */
export function renderTimelineHTML(timeline, title) {
    const html = `
<!DOCTYPE html>
<html>
<head>

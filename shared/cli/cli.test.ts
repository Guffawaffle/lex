...
test('CLI: lex remember with invalid module_scope fails', () => {
  ...
  try {
    ...
    assert.match(error.stderr || error.stdout, /Module validation failed/, 'Should show validation error');
  } finally {
    cleanup();
  }
});
...
test('CLI: lex recall retrieves created frame', () => {
  ...
  try {
    ...
    // Recall it
    const output = execSync(
      `node ${lexBin} recall "recall test frame"`,
      { encoding: 'utf-8', env: { ...process.env, LEX_DB_PATH: testDbPath } }
    );
...
test('CLI: lex recall with --json outputs JSON', () => {
  ...
  try {
    ...
    // Recall it
    const output = execSync(
      `node ${lexBin} --json recall "json recall test"`,
      { encoding: 'utf-8', env: { ...process.env, LEX_DB_PATH: testDbPath } }
    );
...
test('CLI: lex check with no violations exits with 0', () => {
  ...
  try {
    ...
    const output = execSync(
      `node ${lexBin} check ${mergedPath} ${policyPath}`,
      { encoding: 'utf-8' }
    );
...
test('CLI: lex check with --json outputs JSON', () => {
  ...
  try {
    ...
    const output = execSync(
      `node ${lexBin} --json check ${mergedPath} ${policyPath}`,
      { encoding: 'utf-8' }
    );

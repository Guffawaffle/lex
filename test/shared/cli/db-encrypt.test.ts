...
describe("Database Encryption Security (SEC-003)", () => {
  ...
  function getTestEnv(
    dbPath: string,
    passphrase: string = "Test-Passphrase@99"
  ): NodeJS.ProcessEnv {
    return {
      NODE_ENV: "test",
  ...
  describe("Backup Creation", () => {
    test("should create backup by default before encryption", () => {
      ...
      assert.match(
        backupFile,
        /test\.pre-encrypt\.\d{8}-\d{6}\.db/,
        "Backup should have correct format"
      );
    ...
    test("should include backup_path in JSON output when backup is created", () => {
      ...
      assert.match(
        json.backup_path,
        /\.pre-encrypt\.\d{8}-\d{6}\.db$/,
        "backup_path should have correct format"
      );
    });
  });

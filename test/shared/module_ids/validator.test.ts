...
describe("validateModuleIds", () => {
  ...
  test("invalid module IDs fail with error messages", async () => {
    const result = await validateModuleIds(["auth-core", "ui/user-admin-panel"], samplePolicy);
  ...
  test("fuzzy matching suggests similar module names", async () => {
    const result = await validateModuleIds(["auth-core"], samplePolicy);
  ...
  test("multiple invalid modules all reported", async () => {
    const result = await validateModuleIds(["auth-core", "user-panel", "login"], samplePolicy);
  ...
  test("suggestions are limited and relevant", async () => {
    const result = await validateModuleIds(["user-panel"], samplePolicy);
  ...
  test("no suggestions for very dissimilar names", async () => {
    const result = await validateModuleIds(["completely-unrelated-xyz-123"], samplePolicy);
  ...
  test("special characters in module IDs are validated", async () => {
    ...
    const result = await validateModuleIds(["services/auth-core_v2"], policyWithSpecialChars);
  ...
  test("undefined moduleScope is treated as empty", async () => {
    ...
    const result = await validateModuleIds(null as unknown as string[], samplePolicy);
